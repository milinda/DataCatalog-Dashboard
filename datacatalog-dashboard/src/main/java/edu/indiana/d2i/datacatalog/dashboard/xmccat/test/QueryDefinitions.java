 ...
 * returns the descriptions for saved queries.  XMC Cat
 * allows query definitions to be saved for reuse.  When
 * saved, each query is given a description by the user.
 * This query returns those descriptions.  This is used
 * in the XMC Cat GUI to present users with a list of 
 * their saved query definitions. 
 * 
 * @author Scott Jensen scjensen@cs.indiana.edu
 *
 */
public class QueryDefinitions extends TesterBase {
	...
	public static void main(String[] args) throws Exception {
		...
		GetAllQueryDescriptionsRequestDocument doc = GetAllQueryDescriptionsRequestDocument.Factory.newInstance(); 
		GetAllQueryDescriptionsRequestDocument.GetAllQueryDescriptionsRequest request = doc.addNewGetAllQueryDescriptionsRequest();
		// Set the result and error delivery methods
		...
		//invoke service
		GetAllQueryDescriptionsResponseDocument myResponse = stub.getAllQueryDescriptions(doc);
		System.out.println(myResponse.getGetAllQueryDescriptionsResponse().getCatalogOperationStatus().getStatus().toString() );
		System.out.println("\nResponse Document:\n******************\n" + myResponse.toString() );
		if (resultDelivery.equalsIgnoreCase("STREAMING") ) {
			System.out.println("streaming delivery URL:\n" + myResponse.getGetAllQueryDescriptionsResponse().getResultStreamUrl() );
		}
		stub.cleanup();
		return;
	} //end of main
	...
	private static void usage() {
		System.out.println("Usage: QueryDefinitions -dn <dn> -rd <result delivery> -ed <error delivery method> -h <host> -p <port>\n" + 
				"Defaults:\n" + 
				"results = DIRECT\n" +
 			    "errors  = DIRECT\n" +
  			    "host    = localhost\n" + 
